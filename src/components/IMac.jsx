      ...
      <div className="absolute md:top-1/2 top-3/4 md:left-4 left-12 transform -translate-y-1/2 flex flex-col lg:space-y-2 space-y-1 bg-gray-50">
          {imacModels.map((imac, index) => (
            <div key={index} className="flex items-center space-x-2">
              <button style={{'backgroundColor': imac.bg}} className={`2xl:w-8 md:w-5 w-4 2xl:h-8 md:h-5 h-4 rounded-full flex items-center justify-center ${selectedColor === imac.label.toLowerCase() && 'border-2 border-blue-500'}`} onClick={() => {
                setSelectedColor(imac.label.toLowerCase());
                setVisibleLabel(imac.label)
              }} />
              {visibleLabel === imac.label && <span className="text-sm">{imac.label}</span>}
            </div>
          ))}
