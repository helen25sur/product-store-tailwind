    ...
    <div className='flex flex-col items-center justify-around bg-white p-8'>
      ...
      <div className='flex'>
        {macbookModels.map((mb, index) => (
          <button key={index} onClick={() => handleSelectedChip(index)} className={`2xl:w-36 md:w-26 w-28 2xl:h-18 md:h-12 h-12 flex flex-col justify-evenly items-center border ${index === 0 ? 'rounded-l-xl' : 'rounded-r-xl' } ${index === selectedChip ? 'border-2 border-blue-400' : 'border-gray-800'} cursor-pointer`}>
      ...
      <div className="flex flex-col 2xl:p-4 md:items-start items-center">
        <div className="w-80 h-64 2xl:mb-0 mb-8">
          <img src={currentColor.img} alt={`MacBook Air with ${currentMb.chip}`} className='2xl:w-72 xl:w-52 w-48 h-full 2xl:mt-0 sm:mt-6' />
